...
const ItemCard = ({ item, onAdd }) => {
  ...
  const [rateType, setRateType] = useState("weight");

  const handleAdd = () => {
    if (!qty || !rate) return;

    if (Number(qty) > Number(item.remaining_quantity)) {
      return alert("Added quantity is more than remaining quantity.");
    }

    if (rateType === "weight" && !weight) {
      return alert("Please enter a weight for weight-based pricing.");
    }
    ...
    <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 w-full transition-all hover:shadow-md">
      ...
      <div className="mt-4 flex items-center gap-3">
        <select
          ...
          className="px-3 py-2 rounded-xl border text-sm focus:ring-2 focus:ring-blue-300 focus:outline-none"
        >
